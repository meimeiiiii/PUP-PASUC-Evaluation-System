...
public class PECSettingsServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	
    public PECSettingsServlet() {
        super();
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) 
		...
		switch(process) {
	// ---------------------------------- pec-settings?process=editAccount ---------------------------------------------	
			case "editAccount":
...
//								request.getRequestDispatcher("/account?process=getUser").include(request, response);
// 								request.getRequestDispatcher("/pec-pec-settings?process=editAccount").forward(request, response);
 								response.sendRedirect(request.getContextPath() + "/pec-pec-settings?process=editAccount");	
							}else {
									System.out.println("[EDIT] Account edit failed.");
								request.setAttribute("errorMessageEdit","Changes were not saved."); 
								request.getRequestDispatcher("pec-pec-settings?process=editAccount").include(request, response);
							}
		...
		// ---------------------------------- pec-pec-settings?process=changeEmail ---------------------------------------------
			...
			if(saveEmail == null) {			
				...
				if(changeEmailBtn == null) {
					...
					if(acc.changeEmail(email, newEmail) == true) {
						...
						request.getRequestDispatcher("account?process=getUser").include(request, response);
						request.getRequestDispatcher("pec-settings?process=changeEmail").forward(request, response);
					}else {
...
//					break;
	...
	// ---------------------------------- pec-settings?process=changePassword ---------------------------------------------
			case "changePassword":
				...
				if(changePasswordBtn == null) {
					...
					if(oldPassword.equals(newPassword)) {
						...
						if(acc.changePassword(email, oldPassword, newPassword) == true) {
							...
							request.getRequestDispatcher("/account?process=getUser").include(request, response);
							request.getRequestDispatcher("/pec-settings?process=changePassword").forward(request, response);
						}else {
							System.out.println("[CH PW] Account password change failed.");
 					...
 					if(saveBtn == null) {
 						...
 						//String psw = request.getParameter("password");
 							...
 							if(acc.updateUser(email,fn,mn,ln,cn,bd,gen,add,dept) == 0) {
 								...
 								request.setAttribute("errorMessageEdit","Account changes have been saved."); 
// 								request.getRequestDispatcher("/account?process=getUser").include(request, response);
// 								request.getRequestDispatcher("/pec-settings?process=editAccount").forward(request, response);
 								response.sendRedirect(request.getContextPath() + "/pec-settings?process=editAccount");	
								
 							}else {
 									System.out.println("[EDIT] Account edit failed.");
 								request.setAttribute("errorMessageEdit","Changes were not saved."); 
 								request.getRequestDispatcher("/pec-settings?process=editAccount").include(request, response);
 							}
 		...
 		// ---------------------------------- pec-settings?process=changeEmail ---------------------------------------------
 				...
 				if(changePasswordBtn == null) {
 					...
 					if(oldPassword.equals(newPassword)) {
 						...
 						if(acc.changePassword(email, oldPassword, newPassword) == true) {
 							...
 							request.setAttribute("errorMessagePassword","New password has been saved.");
 							//request.getRequestDispatcher("/pec-settings?process=changePassword").forward(request, response);
 							response.sendRedirect(request.getContextPath() + "/pec-settings?process=changePassword");
 						}else {
 							System.out.println("[CH PW] Account password change failed.");
 							request.setAttribute("errorMessagePassword","Changes were not saved."); 
 							request.getRequestDispatcher("/chngPsswrd").include(request, response);
